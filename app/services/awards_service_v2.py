# app/services/awards_service_v2.py
...
def calculate_player_of_the_week_v2(
    ...
    """
    logger.info(f"Starting POTW calculation (v2) for season: {season}, recalculate: {recalculate}")
    ...
    for season_key, weeks in games_by_season_week.items():
        ...
        for week_start, weekly_games in weeks.items():
            ...
            winners = _calculate_week_winners_v2(session, weekly_games, week_start, season_key)
...
def _calculate_week_winners_v2(session: Session, weekly_games: list, week_start: date, season: str) -> list[int]:
    """
...
def get_player_potw_summary(session: Session, player_id: int) -> dict:
    ...
    from app.data_access.crud.crud_player_award import get_player_awards_by_type
...
def migrate_legacy_awards_to_v2(session: Session) -> dict[str, int]:
    """
    Migrate existing player_of_the_week_awards integer values to PlayerAward records.
    This is a one-time migration function.

    Returns:
        Dict with migration statistics
    """
    logger.info("Starting migration of legacy awards to PlayerAward table")

    # Get all players with awards
    from app.data_access.models import Player

    players_with_awards = session.query(Player).filter(Player.player_of_the_week_awards > 0).all()

    migration_stats = {"players_processed": 0, "legacy_awards_found": 0, "records_created": 0, "errors": 0}

    for player in players_with_awards:
        migration_stats["players_processed"] += 1
        legacy_count = player.player_of_the_week_awards
        migration_stats["legacy_awards_found"] += legacy_count

        logger.info(f"Migrating {legacy_count} awards for player {player.name} (ID: {player.id})")

        # For migration, we'll create placeholder records
        # Since we don't have historical week data, we'll create records for recent weeks
        ...
        try:
            ...
            for i in range(legacy_count):
                # Create awards for previous weeks, working backwards
                week_offset = i * 7  # One week apart
                award_date = base_date - timedelta(days=week_offset)
                week_start = award_date - timedelta(days=award_date.weekday())
                award_season = get_season_from_date(award_date)

                # Check if award already exists
                existing = get_awards_by_week(session, "player_of_the_week", week_start, award_season)
                existing_for_player = [a for a in existing if a.player_id == player.id]
                ...
                if not existing_for_player:
                    create_player_award(
                        ...
                        award_type="player_of_the_week",
                        week_date=week_start,
                        points_scored=None,  # Unknown for legacy data
                    )
                    migration_stats["records_created"] += 1

        except Exception as e:
            logger.error(f"Error migrating awards for player {player.id}: {e}")
