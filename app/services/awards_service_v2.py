...
def _calculate_week_winners_v2(
    session: Session, 
    weekly_games: list, 
    week_start: date,
    season: str
    ...
    for player_id in top_players:
        ...
        if player:
            ...
            logger.debug(f"Awarded POTW to {player.name} (ID: {player_id}) for {max_points} points in week {week_start}")
...
def migrate_legacy_awards_to_v2(session: Session) -> dict[str, int]:
    ...
    players_with_awards = session.query(Player).filter(
        Player.player_of_the_week_awards > 0
    ).all()
    
    migration_stats = {
        "players_processed": 0,
        "legacy_awards_found": 0,
        "records_created": 0,
        "errors": 0
    }
