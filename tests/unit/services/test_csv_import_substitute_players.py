...
class TestSubstitutePlayerCSVImport:
    ...
    def test_import_processor_creates_substitute_players(self, db_session):
        ...
        player_data = [
            {"team_name": "Warriors", "name": "Regular Player", "jersey_number": "23", "is_substitute": False},
            {"team_name": "Warriors", "name": "Sub Player 1", "jersey_number": "1", "is_substitute": True},
            {
    ...
    def test_import_processor_handles_duplicate_substitute(self, db_session):
        ...
        existing_sub = Player(name="Sub Player", jersey_number="1", team_id=guest_team.id, is_substitute=True)
        ...
        player_data = [{"team_name": "Any Team", "name": "Sub Player", "jersey_number": "1", "is_substitute": True}]
        ...
        subs = db_session.query(Player).filter_by(name="Sub Player", jersey_number="1", is_substitute=True).all()
        assert len(subs) == 1
    ...
    def test_csv_import_with_optional_fields(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
            writer = csv.writer(f)
            writer.writerow(
                ["team_name", "player_name", "jersey_number", "position", "height", "weight", "year", "is_substitute"]
            )
            writer.writerow(["Warriors", "Complete Sub", "99", "SF", "78", "210", "Senior", "true"])
            f.flush()
    ...
    def test_import_processor_logs_substitute_creation(self, mock_echo, db_session):
        ...
        player_data = [{"team_name": "Any", "name": "New Sub", "jersey_number": "7", "is_substitute": True}]
