...
@pytest.fixture
def team_with_full_data(db_session: Session):
...
class TestTeamDetailPageIntegration:
    ...
    def test_team_detail_page_javascript_functions(self, client, team_with_full_data):
        ...
        for func in required_functions:
            assert f"function {func}" in content or f"{func} = function" in content or f"async function {func}" in content
    ...
    def test_team_player_management_integration(self, client, team_with_full_data, db_session):
        ...
        # Verify player appears in team detail
        detail_response = client.get(f"/v1/teams/{team_with_full_data.id}/detail")
        detail_data = detail_response.json()
        assert len(detail_data["players"]) == 9
        
        # Find the new player
        new_player = next(p for p in detail_data["players"] if p["name"] == "New Player")
        assert new_player["jersey_number"] == "99"
        assert new_player["position"] == "C"
    
    def test_team_stats_with_no_games(self, client, db_session):
