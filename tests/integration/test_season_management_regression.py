    ...
    def test_season_service_import_doesnt_block_startup(self):
        ...
        def track_calls(service_name):
            def wrapper(*args, **kwargs):
                ...
                return type('MockService', (), {
                    'get_or_create_season_from_date': lambda self, date: None
                })()
            return wrapper
        
        # Mock the season service to track if it's called during startup
        with patch('app.services.season_stats_service.SeasonStatsService', side_effect=track_calls('SeasonStatsService')):
            try:
                from app.web_ui.api import app
                client = TestClient(app)
